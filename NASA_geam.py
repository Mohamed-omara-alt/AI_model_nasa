...
class EnhancedSolarDefenderGame:
    ...
    def create_enhanced_visualization(self):
        """Create enhanced professional educational graphics as separate images"""
        if not self.solar_data:
            return

        # Helper function to create and save a single figure
        def create_single_fig(title, figsize=(8, 6)):
            fig, ax = plt.subplots(figsize=figsize, facecolor='#0a0a0a')
            fig.suptitle(title, fontsize=18, color='#00ffff', fontweight='bold', y=0.98)
            return fig, ax

        # 1. Solar Flare Distribution - Enhanced pie chart
        fig1, ax1 = create_single_fig('🎮 Solar Defender - Flare Distribution', figsize=(8, 6))
        self.create_enhanced_pie_chart(ax1)
        plt.tight_layout()
        plt.savefig('solar_flare_distribution.png', dpi=300, facecolor='#0a0a0a')

        # 2. Intensity Timeline - Enhanced
        fig2, ax2 = create_single_fig('🎮 Solar Defender - Intensity Timeline', figsize=(12, 6))
        self.create_intensity_timeline(ax2)
        plt.tight_layout()
        plt.savefig('intensity_timeline.png', dpi=300, facecolor='#0a0a0a')

        # 3. Earth Systems Status - 3D bar chart
        fig3, ax3 = create_single_fig('🎮 Solar Defender - Systems Status', figsize=(8, 6))
        self.create_systems_status(ax3)
        plt.tight_layout()
        plt.savefig('systems_status.png', dpi=300, facecolor='#0a0a0a')

        # 4. Flare Impact Comparison
        fig4, ax4 = create_single_fig('🎮 Solar Defender - Impact Comparison', figsize=(8, 6))
        self.create_impact_comparison(ax4)
        plt.tight_layout()
        plt.savefig('impact_comparison.png', dpi=300, facecolor='#0a0a0a')

        # 5. Performance Gauge
        fig5, ax5 = create_single_fig('🎮 Solar Defender - Performance Gauge', figsize=(6, 6))
        self.create_performance_gauge(ax5)
        plt.tight_layout()
        plt.savefig('performance_gauge.png', dpi=300, facecolor='#0a0a0a')

        # 6. Earth Impact Map
        fig6, ax6 = create_single_fig('🎮 Solar Defender - Earth Impact Map', figsize=(10, 8))
        self.create_earth_impact_map(ax6)
        plt.tight_layout()
        plt.savefig('earth_impact_map.png', dpi=300, facecolor='#0a0a0a')

        # 7. Mission Log
        fig7, ax7 = create_single_fig('🎮 Solar Defender - Mission Log', figsize=(8, 6))
        self.create_mission_log(ax7)
        plt.tight_layout()
        plt.savefig('mission_log.png', dpi=300, facecolor='#0a0a0a')

        # Optionally show them if needed, but for API, probably remove
        # plt.show()
